...
class AddFarmersSuccessStoriesScreen extends StatefulWidget {
  @override
  _AddFarmersSuccessStoriesScreenState createState() =>
      _AddFarmersSuccessStoriesScreenState();
}

class _AddFarmersSuccessStoriesScreenState
    extends State<AddFarmersSuccessStoriesScreen> {
  String spotName = '';
  String spotImageURL = '';
  ...
  void submitFarmersSuccessStories() async {
    ...
    try {
      if (spotImageURL.isNotEmpty) {
        ...
        final String storageResponse =
            await supabase.storage.from('farmer_stories_images').upload(
                  filename,
                  File(spotImageURL),
                  fileOptions:
      ...
      final response = await supabase.from('farmer_success_stories').upsert([
        spotData,
      ]);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Add Stories Spot'),
        centerTitle: true,
      ),
      body: Padding(
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 16),
              ElevatedButton(
                onPressed: submitFarmersSuccessStories,
